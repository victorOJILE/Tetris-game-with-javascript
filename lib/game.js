...
let interval, level, score = 0,
 re_render, scoreAdder, randNum, resume, new_game;
...
const Tetris = (function() {
 ...
 async function moveBox() {
  ...
  if (boxOccupied(boxes) || hitBottom(activeBox)) {
   iterate(boxes, e => e.box.y--);
 ...
 return {
  ...
  controlBlockChange() {
   changeShapes[`change${activeBox.name}`](deepCopy(activeBox.boxes));
  },
  throwDown(mainObj) {
   let boxes = activeBox.boxes;
   ...
   function recall() {
    ...
    if (boxOccupied(boxes) || boxes.find(obj => obj.box.y == 28)) {
     iterate(boxes, obj => obj.box.y--);
  ...
  moveRight() {
   ...
   boxOccupied(activeBox.boxes) && iterate(activeBox.boxes, obj => obj.box.x--) || rerenderGame(activeBox);
  },
  moveLeft() {
