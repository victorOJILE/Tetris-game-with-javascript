const elCls = function(cls) { 
	return document.getElementsByClassName(cls);
}

const elName = function(name) {
	return document.getElementsByName(name);
}

const elId = function(id) {
...
const Tetris = (function() {
 ...
 const iterate = function(iterable, func) {
 ...
 let context = localStorage.getItem('context'), activeBox, ctx;
...
const savedGame = gameState.getContext('2d');
 ...
 const utils = {
  ...
  addScore() {
   try {
    ...
    let counts = re_render.reduce(function(obj, box) {
    ...
    for (let key in counts) {
     if (counts[key] == 14) { // if the y (stored in key) line is filled
      re_render = re_render.filter(function(each){ return each.box.y != key }); // clear out all boxes on that y line
      re_render.forEach(function(each) {
       if (each.box.y >= key) return;
       each.box.y++; // shift all boxes down if possible else return (continue)
      });
      ctx.clearRect(0, 0, canvas_width, canvas_height);
      iterate(re_render, function(obj){ draw(obj) });
      utils.saveState();
  ...
  hitBottom(mainObj) {
   return mainObj.boxes.find(function(obj){ return obj.box.y == 28 });
  },
  deepCopy(obj) {
 ...
 async function moveBox() {
  ...
  iterate(boxes, function(e){ return e.box.y++ });
 ...
 async function showNextBox() {
  ...
  iterate(shapes[shapesArr[randNum]](true), function(obj){ draw2(obj) });
 }
 ...
 function rerenderGame(shape) {
 	...
 	iterate(shape.boxes, function(key){	draw(key) });
 	activeBox = shape;
 }
 ...
 let changeShapes = {
  ...
  changeVertL(oldShape) {
   ...
   boxes[1].box.x--;
   boxes[3].box.x += 1;
   iterate(boxes, function(obj){ obj.box.y = boxes[1].box.y });
  ...
  changeHorizL(oldShape) {
   ...
   boxes[1].box.y -= 1;
   boxes[3].box.y += 1;
   iterate(boxes, function(obj){ obj.box.x = boxes[2].box.x });
   if (utils.boxOccupied(boxes)) {
    activeBox.boxes = oldShape;
    return;
   }
  ...
  changePlusDown(oldShape) {
   ...
   boxes[2].box.y += 1;
   boxes[3].box.y -= 1;
   iterate(boxes, function(obj){ obj.box.x = boxes[1].box.x });
   boxes[3].box.x -= 1;
   if (utils.boxOccupied(boxes)) {
    activeBox.boxes = oldShape;
    return;
  ...
  changeBlockDownRight(oldShape) {
   ...
   boxes[1].box.y -= 2;
   boxes[2].box.y -= 1;
   iterate(boxes, function(obj){ obj.box.x = boxes[0].box.x });
   boxes[0].box.x -= 1;
   if (utils.boxOccupied(boxes)) {
    activeBox.boxes = oldShape;
    return;
  ...
  changeBlockUpLeftDown(oldShape) {
   ...
   boxes[2].box.x -= 1;
   boxes[3].box.x -= 2;
   iterate(boxes, function(obj){ obj.box.y = boxes[3].box.y });
   boxes[0].box.y -= 1;
   if (utils.boxOccupied(boxes)) {
    activeBox.boxes = oldShape;
    return;
 ...
 return {
  ...
  throwDown(mainObj) {
   let boxes = activeBox.boxes;
   function recall() {
   	iterate(boxes, function(obj){ obj.box.y++ });
    if (utils.boxOccupied(boxes) || boxes.find(function(obj){ return obj.box.y == 28 })) {
    	iterate(boxes, function(obj){ obj.box.y-- });
  ...
  moveRight() {
   iterate(activeBox.boxes, function(obj){
    obj.box.x++ });
   for (let obj of activeBox.boxes) {
    if (obj.box.x > 13) return iterate(activeBox.boxes, function(obj){ obj.box.x-- });
   }
   utils.boxOccupied(activeBox.boxes) &&	iterate(activeBox.boxes, function(obj){ obj.box.x-- }) || rerenderGame(activeBox);
  },
  moveLeft() {
  	iterate(activeBox.boxes, function(obj){ obj.box.x-- });
   for (let obj of activeBox.boxes) {
    if (obj.box.x < 0) return iterate(activeBox.boxes, function(obj){ obj.box.x++ });
   }
   utils.boxOccupied(activeBox.boxes) && iterate(activeBox.boxes, function(obj){ obj.box.x++ }) || rerenderGame(activeBox);
  },
  showNextBox,
